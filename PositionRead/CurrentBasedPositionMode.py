...
set_A_PRFL = 10      # between 0 ~ set_A_limit      [214.577 rev/min^2]
...
dxl_comm_result, dxl_error = packetHandler.write4ByteTxRx(portHandler, DXL4_ID, ADDR_PRO_GOAL_POSITION, int(final_pos_step))
if dxl_comm_result != COMM_SUCCESS:
    print("%s" % packetHandler.getTxRxResult(dxl_comm_result))
elif dxl_error != 0:
    print("%s" % packetHandler.getRxPacketError(dxl_error))
...
try:
    while True:
        ...
        dxl_current_limit, dxl_comm_result, dxl_error = packetHandler.read2ByteTxRx(portHandler, DXL4_ID, ADDR_PRO_CURRENT_LIMIT)
        if dxl_comm_result != COMM_SUCCESS:
            print("%s" % packetHandler.getTxRxResult(dxl_comm_result))
        elif dxl_error != 0:
            print("%s" % packetHandler.getRxPacketError(dxl_error))
        ...
        dxl_present_position, dxl_comm_result, dxl_error = packetHandler.read4ByteTxRx(portHandler, DXL4_ID, ADDR_PRO_PRESENT_POSITION)
        angle = map(dxl_present_position, 0, 4095, 0.0, 360.0)
        ...
        print("Present Position [step]: %d" %dxl_present_position)
        print("Present Position [deg]: %f" %angle)
